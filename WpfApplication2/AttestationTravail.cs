...
namespace WindowWPf
{
    static class AttestationTravail
    {
    

        public static void genererAttest(Employe e)
        {
            var entreprise = Variables.db.Parametres.FirstOrDefault();
            Document document = new Document();
            document.LoadFromFile($@"{Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location)}\Resources\REF-ATTESTATION DE TRAVAIL.docx");
            ...
            if (e.DateDeNaissance != null) document.Replace("DATE_DE_NAISSANCE",e.DateDeNaissance.Value.ToShortDateString(),true,true);
            ...
            document.Replace("POSTE",e.Poste,true,true);
